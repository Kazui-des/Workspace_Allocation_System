...
def get_workspace_bookings(workspace_id):
    ...
    # Add unique class routines with division
    seen_classes = set()
    for r in routines:
...
def add_workspace():
    ...
    if request.method == 'POST':
        ...
        if not name or not capacity or not amenities:
            flash('All fields are required.', category='error')
            return redirect(url_for('views.add_workspace'))

        # Check if workspace already exists
        existing_workspace = Workspace.query.filter_by(name=name).first()
        if existing_workspace:
            flash('Workspace with this name already exists.', category='error')
            return redirect(url_for('views.add_workspace'))

        # Add new workspace
        new_workspace = Workspace(name=name, capacity=int(capacity), amenities=amenities)
        db.session.add(new_workspace)
        db.session.commit()
        flash('Workspace added successfully!', category='success')
        return render_template("dashboard.html", user=current_user)

    return render_template("add-workspace.html", user=current_user)
